...
import { $, esc, api, startPolling, toast, mountNav, tryWarden, savedPin, wardenModal, ask, tell } from './common.js';
...
function renderFight() {
  ...
  if (!c.active) {
    ...
    box.querySelector('[data-start]')?.addEventListener('click', () => act({ action: 'start' }, 'Combat begins.'));
    return;
  }
...
function renderEnemies() {
  const list = combat.enemies.filter((e) => !e.defeated);
  ...
  $('#enemies').innerHTML = list.length ? list.map((e) => `<div class="run-row${combat.combat?.current === e.id ? ' now' : ''}">
      <div class="run-who"><b>${esc(e.name)}</b>${e.frenzied?.length ? '<span class="run-st hot">FRENZIED</span>' : ''}${e.submerged ? '<span class="run-st">submerged</span>' : ''}${statusTags(e.statuses)}</div>
...
function renderPosse() {
  ...
  $('#posse').innerHTML = list.length ? list.map((p) => {
    ...
    return `<div class="run-row${combat.combat?.current === p.id ? ' now' : ''}${p.bleeding ? ' bleed' : ''}">
      <div class="run-who"><a href="/posse#${esc(p.id)}"><b>${esc(p.name)}</b></a><small>${esc(p.trade)}${p.player ? ` · ${esc(p.player)}` : ''}</small>
