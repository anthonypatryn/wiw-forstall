...
export const WHOLE_MAP = 999; // Town Forstall: the town perimeter covers the whole board
export const KINDS = {
...
export function fields(battle, combat) {
  ...
  for (const pc of combat?.posse || []) {
    ...
    out.push({
      ...
      charges: Number(pc.forstall.charges) || 0, sweep: sweeps[key] || null,
    });
  }
  for (const f of battle?.forstalls || []) {
    ...
    out.push({
      ...
      slots: (f.slots || []).slice(0, SLOTS), known: knownOf(f.slots), fuse: true, charges: null, sweep: sweeps[f.id] || null,
    });
  }
  return out;
}
...
// Monsters a Forstall can be programmed with: every one for the Warden, only decoded ones for the posse (Scanner notebook).
export function programmable(scan, warden) {
  const custom = (scan?.custom || []).map((m) => ({ name: m.name, kz: m.kz }));
  const all = [...MONSTERS.map((m) => ({ name: m.name, kz: m.kz })), ...custom].sort((a, b) => a.name.localeCompare(b.name));
  if (warden) return all;
  const solved = new Set(Object.entries(scan?.notebook || {}).filter(([, e]) => e.solved).map(([n]) => n));
  return all.filter((m) => solved.has(m.name));
}
