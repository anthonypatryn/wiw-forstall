...
export function sessionAction(state, a) {
  ...
  switch (a.action) {
    case 'add': {
      ...
      // `created` may reach back (up to 2 days) so a session started at the end of the night still covers the night's log
      const now = Date.now(), from = Math.round(Number(a.created) || now);
      const s = { id: id(), title: clean(a.title, 80) || `Session ${n}`, date: clean(a.date, 20), notes: '', recap: '', at: now, created: Math.max(now - 2 * 86400e3, Math.min(now, from)) };
    ...
    case 'end': { // End Session: the night is over
      const s = find();
      s.ended = Date.now(); s.at = s.ended;
      return s;
    }
    case 'remove':
      find();
