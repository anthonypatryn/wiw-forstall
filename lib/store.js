...
async function commitRaw(writes) { // writes: [{ key, raw, expect }]
  ...
  if (storeKind === 'redis') {
    let ok;
    try { ok = await redis(['EVAL', CAS, String(writes.length), ...writes.map((w) => redisKey(w.key)), ...writes.flatMap((w) => [w.expect, w.raw])]); }
      ...
      await Promise.all(writes.map((w) => redis(['SET', redisKey(w.key), w.raw])));
      return true;
    }
    return Number(ok) === 1;
  }
