...
// try again by paying the cost the Warden set (up to the Warden's retry limit): fresh deck, same peeks as the first Finesse roll.
...
const STATUS = ['Afraid', 'Burned', 'Dazed', 'Electrocuted', 'Poisoned', 'Trapped', 'Unconscious'];
// what's inside (one choice) and whether the box is trapped (hurts whoever opens it)
function cleanLoot(l) {
  if (!l || !['money', 'scrap', 'item', 'custom'].includes(l.kind)) return null;
  if (l.kind === 'money' || l.kind === 'scrap') { const amount = Math.max(0, Math.round((Number(l.amount) || 0) * 100) / 100); return amount ? { kind: l.kind, amount } : null; }
  if (l.kind === 'item') return l.itemId ? { kind: 'item', itemId: clean(l.itemId, 80), name: clean(l.name, 80) } : null;
  const name = clean(l.name, 80);
  return name ? { kind: 'custom', name, desc: clean(l.desc, 300) } : null;
}
function cleanTrap(t) {
  if (!t) return null;
  const damage = int(t.damage, 0, 30), status = STATUS.includes(t.status) ? t.status : '', sev = int(t.sev, 1, 6);
  return damage || status ? { damage, status, sev: status ? sev : 0 } : null;
}
const lootText = (l) => (!l ? '' : l.kind === 'money' ? `$${l.amount}` : l.kind === 'scrap' ? `${l.amount} Scrap` : l.name);
const trapText = (t) => (!t ? '' : [t.damage ? `−${t.damage} Health` : '', t.status ? `${t.status} [${t.sev}]` : ''].filter(Boolean).join(', '));

export function lockAction(state, x, { warden, names = {}, rollFinesse, log = () => {}, deck, onOpen = () => {} }) {
  ...
  switch (x.action) {
    case 'start': { // the Warden hands one or more players a lock
      ...
      const loot = cleanLoot(x.loot), trap = cleanTrap(x.trap);
      const made = who.map((pc) => ({ id: crypto.randomUUID().slice(0, 8), pc, name: names[pc], what, need, retriesLeft: retries, retryCost: cost, tries: 1, peeks: 0, status: 'finesse', at: Date.now(), wins: 0, history: [], loot, trap }));
    ...
    case 'finesse': { // roll Finesse first: each Hit = one peek at the next card's color
      ...
      a.peeks = a.rolledPeeks = r.hits; // retries get the same number of peeks without rolling again
      deal(a, deck?.() || newDeck());
    ...
    case 'guess': {
      ...
      if (a.wins >= a.need) {
        a.status = 'picked';
        // open it: the trap goes off, then the loot is theirs
        if (a.trap) { onOpen(a, 'trap'); a.sprung = trapText(a.trap); }
        if (a.loot) { onOpen(a, 'loot'); a.found = lootText(a.loot); }
        log(`Click! ${a.name} picks ${a.what} (${a.need} in a row).${a.sprung ? ` It was trapped — ${a.sprung}!` : ''}${a.found ? ` Inside: ${a.found}.` : ''}`);
      }
    ...
    case 'retry': {
      ...
      // no second Finesse roll: a fresh deck, and the peeks their first roll earned
      a.retriesLeft -= 1; a.tries += 1; a.peeks = a.rolledPeeks ?? 0;
      deal(a, deck?.() || newDeck());
      log(`${a.name} ${a.retryCost ? `pays ${a.retryCost} and ` : ''}tries ${a.what} again.`);
      return;
    }
    case 'giveUp': { const a = find(); mine(a); if (a.status === 'picked') a.closed = true; else if (a.status === 'failed') { if (a.retriesLeft > 0) log(`${a.name} gives up on ${a.what}.`); a.closed = true; } else { a.status = 'failed'; a.closed = true; log(`${a.name} leaves ${a.what} be.`); } return; }
...
export function lockView(state, { warden, pc }) {
  // what's inside (and the trap) stays a surprise until it opens
  const pub = ({ deck, loot, trap, ...a }) => ({ ...a, ...(warden ? { loot, trap } : {}), left: deck ? deck.length : null });
