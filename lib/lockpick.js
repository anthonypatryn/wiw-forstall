...
export const lootText = (l) => (!l ? '' : l.kind === 'money' ? `$${l.amount}` : l.kind === 'scrap' ? `${l.amount} Scrap` : l.name);
export const trapText = (t) => (!t ? '' : [t.damage ? `−${t.damage} Health` : '', t.status ? `${t.status} [${t.sev}]` : ''].filter(Boolean).join(', '));
