...
export function wardenNeeds(state, { shop, battle, edison, whispers }) {
  ...
  for (const ck of (state.checks || []).filter((x) => !x.winner)) {
    const waiting = ck.who.filter((pid) => !ck.rolls[pid]).map(name);
    out.push({ urgent: !waiting.length, href: '/run', text: waiting.length ? `${ck.skill} ${ck.kind === 'challenge' ? 'Challenge' : 'check'} — waiting on ${waiting.join(', ')}` : `${ck.skill} check — everyone rolled, close it` });
  }
