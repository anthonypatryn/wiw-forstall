...
export function wardenNeeds(state, { shop, battle, edison }) {
  ...
  for (const ck of (state.checks || []).filter((x) => !x.winner)) {
    ...
    out.push({ urgent: !waiting.length, href: '/session', text: waiting.length ? `${ck.skill} ${ck.kind === 'challenge' ? 'Challenge' : 'check'} — waiting on ${waiting.join(', ')}` : `${ck.skill} check — everyone rolled, close it` });
  }
