...
function forstallAction(state, a, { warden, list, tokens, cave }) {
  ...
  if (!f && a.op !== 'edison') throw new Error('That Forstall isn’t in play.');
  const pc = f?.owner ? findPc(state, f.owner) : null;
  if (f && !f.owner && !warden) throw new Error('Only the Warden runs that Forstall.');
  const c = state.combat;
  ...
  switch (a.op) {
    case 'sweep': {
      ...
      const label = `${f.name} — ${again ? 'readjusting the Sweep' : 'Sweeping'}${cave ? ' (cave, −1 die)' : ''}`;
