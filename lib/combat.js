...
import { hexDist, inRange, bestSweep, lossText, touching } from './forstall.js';
...
function wardenCombat(state, a) {
  ...
  switch (a.action) {
    ...
    case 'end':
      c.active = false; c.round = 0; c.slots = []; c.current = null; c.init = {}; c.surprise = []; c.tiebreak = {}; state.emp = null; state.scanRound = null; c.party = null;
...
function forstallAction(state, a, { warden, list, tokens, cave }) {
  ...
  const pc = f?.owner ? findPc(state, f.owner) : null;
  if (f && !f.owner && !warden && a.op !== 'emp') throw new Error('Only the Warden runs that Forstall.');
  const c = state.combat;
  // In a fight a character works their Forstall on their own turn (the Warden can always step in)
  ...
  switch (a.op) {
    case 'sweep': {
      if (pc) {
        const ch = Number(pc.forstall.charges) || 0;
      ...
      if (pc) pc.forstall.charges = Math.max(0, (Number(pc.forstall.charges) || 0) - 1);
      ...
      let eff = '';
      if (a.efficiency && pc && (pc.abilities || []).includes('Forstall Efficiency')) {
        const used = pc.abilityUses?.['Forstall Efficiency'] || 0;
        const d = r.dice.find((x) => x.face === 'hit');
        if (used >= 2) throw new Error('Forstall Efficiency is used up for today (2/day).');
        if (d) { d.face = 'ace'; r.hits += 1; r.aces = (r.aces || 0) + 1; pc.abilityUses = { ...(pc.abilityUses || {}), 'Forstall Efficiency': used + 1 }; eff = ' + Forstall Efficiency'; }
      }
      ...
      return { dice: r.dice, hits: r.hits, pool: poolLabel(pool), label, charges: pc ? pc.forstall.charges : null };
    }
    case 'off':
    ...
    case 'burst': {
      ...
      let gi = -1;
      if (pc) {
        gi = pc.gear.findIndex((g) => /crystal/i.test(g.item || ''));
        if (gi < 0) throw new Error(`${pc.name} has no Refined Crystal in their gear — each Burst shatters one.`);
      }
...
export function playerCombatView(state) {
  ...
  return {
    ...
    enemies: state.enemies.map((e) => ({
      id: e.id, name: e.name, size: e.size, statuses: e.statuses, defeated: e.defeated, out: !!e.out, frenzied: e.frenzied.length > 0, grit: e.grit, speed: e.speed, turnLog: e.turnLog || [], crippled: !!e.crippled,
