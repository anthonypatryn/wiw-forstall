...
function pcOp(state, a) {
  ...
  switch (a.op) {
    ...
    case 'attack': { // p. 41: spend the weapon's Grit, roll its pool at that Range; damage = Hits − target's Defense (p. 43)
      ...
      log(state, { type: 'roll', who: pc.name, label: `${wname} → ${e.name} · ${RANGE_NAME[rng]}${a.dist != null ? ` (${a.dist}″)` : ''}${aim ? ' · Aim' : ''}${ammo ? ` · ${ammo}` : ''}`, pool: poolLabel(pool), spur, dice: r.dice, hits, aces: r.dice.filter((d) => d.face === 'ace').length });
