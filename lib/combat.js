...
function duelist(state, ref) {
  const [kind, rid] = String(ref || '').split(':');
...
function duelAction(state, a) {
  ...
  switch (a.action) {
    case 'duelStart': {
      ...
      if (!A || !B || A.who === B.who) throw new Error('Pick two different duelists.');
    ...
    case 'duelRoll': {
      ...
      if (skill !== 'Draw!') {
        ...
        d.result = pcs.map((pc, i) => {
          ...
          if (sides[i].kind === 'en') { // enemies don't Bleed Out (p. 55): a bad Draw! puts them down
