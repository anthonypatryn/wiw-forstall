...
function pcOp(state, a) {
  ...
  switch (a.op) {
    ...
    case 'useItem': { // p. 42: spend the item's Grit and roll its dice
      ...
      spendGrit(state, pc, cost, `Using ${g.item}`);
    ...
    case 'improvise': case 'prepare': { // p. 42–43: Improvise costs 1+ Grit (maybe a Skill roll); Prepare holds an Action, once per turn
...
export function playerCombatView(state) {
  ...
  return {
    ...
    enemies: state.enemies.map((e) => ({
      id: e.id, name: e.name, size: e.size, statuses: e.statuses, defeated: e.defeated, frenzied: e.frenzied.length > 0, grit: e.grit, speed: e.speed, turnLog: e.turnLog || [],
