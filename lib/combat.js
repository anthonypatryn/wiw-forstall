...
export function wardenNeeds(state, { shop, battle, edison, whispers }) {
  ...
  for (const ck of state.checks || []) {
    ...
    if (ck.kind === 'challenge') {
      out.push(ck.winner ? { urgent: true, href: '/run#grp-now', closeCheck: ck.id, text: `${ck.skill} Challenge is in: ${ck.winner} wins. Close it?` }
        : { urgent: false, href: '/run#grp-now', text: `${ck.skill} Challenge — waiting on ${waiting.join(', ') || 'a reroll'}` });
      continue;
    }
    if (waiting.length) { out.push({ urgent: false, href: '/run#grp-now', text: `${ck.skill} check — waiting on ${waiting.join(', ')}` }); continue; }
    ...
    out.push({ urgent: true, href: '/run#grp-now', closeCheck: ck.id, text: `${ck.skill} check is in: ${res.join(', ')}. Close it?` });
  }
