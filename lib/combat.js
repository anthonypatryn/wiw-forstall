...
import { newSheet, setSheetField, toggleSheetList, hasSpur, giveStartingWeapons, weaponFields, gearFields, tierRanged, tierMelee, upgradeFits, mechState, blankWeapon, blankGear, blankHorse, blankForstall, blankMech, attachUpgrade, unequipItem, forstallFields, mechFields, horseFields, placedIn, TIERS, ACHIEVEMENTS, EXTRA_CATS, STARTING_WEAPONS, TALENTS, REPUTATION, MECH_STATES, PACKS, KEEPSAKES } from './sheets.js';
import { CATALOG } from './catalog.js';
import { TROPHIES, CONDITIONS, CONDITION_GUIDE, trophyValue, trophyName } from './trophies.js';
import { hexDist, inRange, bestSweep, lossText, touching, sweepTolerance } from './forstall.js';
import { id, int, clean as cleanText } from './util.js';
...
const CAPTURE = { Tiny: 6, Small: 8, Human: 8, Medium: 10, Large: 12, Huge: 14, Titan: null };
export const SKILLS = ['Charm', 'Finesse', 'Intuition', 'Nerve'];
const GRIT = 6;
const MAX_LOG = 80;

const now = () => Date.now();
const clean = (s, n = 40) => cleanText(s, n);
...
function log(state, entry) {
  const e = { id: id(), at: now(), ...entry };
  if (!state.archive) state.archive = [...(state.log || [])].reverse().map(archived); // first time: start from what's already in the log
  state.log = [e, ...state.log].slice(0, MAX_LOG);
  // the Table Log only keeps the last 80; the archive keeps a whole session for the Warden's write-up (never sent to players)
  (state.archive ||= []).push(archived(e));
  if (state.archive.length > ARCHIVE_MAX) state.archive = state.archive.slice(-ARCHIVE_MAX);
}
const ARCHIVE_MAX = 1000; // a long session logs a few hundred; keeps the combat document (loaded on every change) well under Redis limits
const archived = (e) => ({ id: e.id, at: e.at, text: e.type === 'roll'
  ? `${e.who} rolled ${e.label || 'dice'} (${e.pool}) → ${e.hits} Hit${e.hits === 1 ? '' : 's'}`
  : String(e.text || ''), ...(e.secret ? { secret: e.secret } : {}), ...(e.hidden ? { hidden: true } : {}) });
...
// Who's in this fight: the Warden picks at the start (combat.party = posse ids; enemies sitting it out have e.out). No list = everyone.
const inFight = (state, x) => (findPc(state, x.id) ? !state.combat.party || state.combat.party.includes(x.id) : !x.out);
function fullOrder(state) {
  const list = [];
  for (const slot of state.combat.slots) {
    if (slot === 'enemies') state.enemies.filter((e) => !e.out).forEach((e) => list.push(e.id));
    else if (findPc(state, slot)) list.push(slot);
  }
  return list;
}
const canAct = (state, key) => { const p = findPc(state, key); if (p) return !p.dead && inFight(state, p); const e = findEnemy(state, key); return !!e && !e.defeated && !e.out; };
...
const DIFFICULTY = [['Very Easy', 1], ['Easy', 2], ['Medium', 3], ['Difficult', 4], ['Very Difficult', 5]];
...
function resolveChallenge(state, ck) {
  ...
  if (top.length > 1) { // "the struggle ain't over yet" — everyone rolls again
    ...
    log(state, { type: 'event', text: `Tie at ${best} (${top.map((x) => x.name).join(' & ')}) — the struggle ain’t over. Roll ${ck.skill} again!` });
  } else {
...
const UNDO_ACTIONS = new Set(['enemyAttack', 'next', 'roll', 'enemy', 'checkRoll', 'forstall', 'join', 'leave']);
...
// Parts of a character sheet a fight never changes (story text, inventory, Prestige…): left out of undo snapshots so they
// stay small, and left alone when an undo puts the rest back — so a purchase or sheet edit mid-fight isn't undone with it.
const UNDO_SKIP = new Set(['disposition', 'appearance', 'history', 'inventory', 'items', 'reputation', 'player', 'prestige', 'talents', 'achievements', 'title',
  'keepsakes', 'pack', 'pack2', 'tier', 'tierApplied', 'tierKit', 'done', 'portrait', 'sell', 'trade', 'skills', 'abilities', 'aceTwo', 'story']);
const fightPart = (pc) => Object.fromEntries(Object.entries(pc).filter(([k]) => !UNDO_SKIP.has(k)));
export function pushUndo(state, label = '', extra = {}) {
  const snap = JSON.parse(JSON.stringify({ posse: state.posse.map(fightPart), enemies: state.enemies, combat: state.combat, duel: state.duel ?? null, checks: state.checks ?? [], sweeps: state.sweeps ?? {} }));
...
export function undoLabel(state, a) {
  if (a.action === 'forstall') return `Forstall: ${{ sweep: 'Sweep', off: 'switch off', burst: 'Burst', edison: 'Edison’s Rule 1' }[a.op] || a.op}`;
...
export function undoCombat(state, { warden, mode }) {
  ...
  const { posse: snapPosse, ...rest } = oldest.snap;
  Object.assign(state, rest);
  // characters: put back only what the fight changes; everything else stays as it is now (and anyone added since stays)
  const byId = new Map(snapPosse.map((p) => [p.id, p]));
  state.posse = state.posse.map((pc) => (byId.has(pc.id) ? { ...pc, ...byId.get(pc.id) } : pc));
  const idx = oldest.logTop ? state.log.findIndex((l) => l.id === oldest.logTop) : state.log.length;
  if (idx > 0) { const gone = new Set(state.log.slice(0, idx).map((l) => l.id)); state.archive = (state.archive || []).filter((l) => !gone.has(l.id)); state.log = state.log.slice(idx); }
...
// ---------- Range from the battle map (pp. 40 & 85): when both tokens are on the board, the real distance decides ----------
const BAND_ORDER = ['arms', 'short', 'long', 'distant'];
const bandOf = (d) => (d <= 1 ? 'arms' : d <= 6 ? 'short' : d <= 18 ? 'long' : 'distant');
const hexD = (a, b) => { const c = (col, row) => { const q = col - (row - (row & 1)) / 2; return [q, row, -q - row]; }; const A = c(a.col, a.row), B = c(b.col, b.row); return Math.max(...A.map((v, i) => Math.abs(v - B[i]))); };
// Rewrites body.range to the band the target is really in, or throws if nothing reaches. The Warden can force it.
export function applyMapRange(state, tokens, body, warden) {
  if (!state.combat?.active || !Array.isArray(tokens)) return;
  const tokOf = (ref) => tokens.find((t) => t.ref === ref);
  const force = warden && body.force;
  if (body.action === 'pc' && (body.op === 'attack' || body.op === 'fireHold')) {
    const pc = findPc(state, body.id);
    if (!pc) return;
    const hold = body.op === 'fireHold' ? pc.hold : null;
    if (body.op === 'fireHold' && hold?.kind !== 'attack') return;
    const targetId = body.target || hold?.triggeredBy?.enemy || hold?.trigger?.enemy;
    const me = tokOf(pc.id), foe = tokOf(targetId);
    if (!me || !foe) return; // not both on the board: the chosen range stands
    const d = hexD(me, foe), band = bandOf(d);
    const w = pc.weapons[int(hold ? hold.weapon : body.weapon, 0, 2)] || {};
    if (POOLRE.test(String(w[band] || ''))) { body.range = band; body.dist = d; return; }
    if (force) { body.dist = d; return; }
    throw new Error(`${findEnemy(state, targetId)?.name || 'The target'} is ${d}″ away (${RANGE_NAME[band]}) — ${w.model || w.manufacturer || 'that weapon'} has no dice at that range.`);
  }
  if (body.action === 'enemyAttack') {
    const e = findEnemy(state, body.enemy), pc = findPc(state, body.pc);
    const at = profileFor(e?.profile)?.attacks?.[int(body.attack, 0, 99)];
    const me = e && tokOf(e.id), foe = pc && tokOf(pc.id);
    if (!at || !me || !foe) return;
    const d = hexD(me, foe), band = bandOf(d);
    const reach = { Melee: 0, Short: 1, Long: 2, Distant: 3 }[at.range] ?? 3;
    body.dist = d;
    if (BAND_ORDER.indexOf(band) <= reach || force) return;
    throw new Error(`OUT_OF_RANGE: ${pc.name} is ${d}″ away (${RANGE_NAME[band]}) — ${at.name} only reaches ${at.range === 'Melee' ? 'Arm’s Reach' : `${at.range} Range`}.`);
  }
}
...
function prepareHold(state, pc, a) {
  ...
  } else if (h.kind === 'improvise') {
    hold.grit = int(h.grit, 1, 12); hold.text = clean(h.text, 60) || 'something';
    hold.skill = SKILLS.find((x) => x === h.skill) || '';
    cost = hold.grit; label = `improvise: ${hold.text}${hold.skill ? ` (${hold.skill})` : ''}`;
...
function fireHold(state, pc, a) {
  ...
  if (h.kind === 'attack') {
    ...
    res = pcOp(state, { action: 'pc', id: pc.id, op: 'attack', weapon: h.weapon, range: a.range || h.range, dist: a.dist, target, ammo: h.ammo, aim: h.aim, prepaid: true });
  } else if (h.kind === 'dodge') {
    ...
    if (h.skill) {
      const sp = parsePool(pc.skills[h.skill.toLowerCase()] || ''), r = rollPool(sp.black, sp.gold, hasSpur(pc, h.skill));
      log(state, { type: 'roll', who: pc.name, label: `Prepared: ${h.text} · ${h.skill}`, pool: poolLabel(sp), spur: hasSpur(pc, h.skill), dice: r.dice, hits: r.hits, aces: r.aces });
      res = state.log[0];
    }
  }
...
function orderSlots(state) {
  const c = state.combat;
  const keys = [...state.posse.filter((p) => !p.dead && inFight(state, p)).map((p) => p.id), ...(state.enemies.some((e) => !e.out) ? ['enemies'] : [])];
...
function enemyFinessePool(state) {
  const lead = state.enemies.find((e) => !e.defeated && !e.out) || state.enemies[0];
...
function townRest(state, pc, quiet) {
  ...
  state.enemies.forEach((e) => { e.empUses = 0; });
...
// a player asks to call out an NPC from the ledger; the Warden accepts (picks how tough, p. 191) or says no
const DUEL_TOUGH = ['Weak', 'Moderate', 'Strong'];
function duelRequestAction(state, a, warden) {
  state.duelRequests ||= [];
  if (a.action === 'duelRequest') {
    const pc = findPc(state, clean(a.pc, 12));
    if (!pc || pc.dead) throw new Error('Pick who you’re playing first (the “This is me” star on your sheet).');
    const npcName = clean(a.npcName, 60);
    if (!npcName) throw new Error('Who are you calling out?');
    if (state.duelRequests.some((r) => r.pc === pc.id && r.status === 'pending')) throw new Error('You already have a challenge waiting on the Warden.');
    const r = { id: crypto.randomUUID().slice(0, 8), pc: pc.id, name: pc.name, npcId: clean(a.npcId, 12), npcName, reason: clean(a.reason, 140), status: 'pending', note: '', at: now() };
    state.duelRequests = [r, ...state.duelRequests].slice(0, 20);
    return { id: r.id };
  }
  if (!warden) throw new Error('Warden PIN required.');
  const r = state.duelRequests.find((x) => x.id === a.id);
  if (!r || r.status !== 'pending') throw new Error('That challenge has already been answered.');
  if (a.accept) {
    const tough = DUEL_TOUGH.includes(a.tough) ? a.tough : 'Moderate';
    duelAction(state, { action: 'duelStart', a: `pc:${r.pc}`, b: `np:npc:Human - ${tough} Combatant|${r.npcName}` });
    Object.assign(r, { status: 'accepted', tough, answeredAt: now() });
    if (r.reason) log(state, { type: 'event', text: `${r.name} called ${r.npcName} out: “${r.reason}”` });
  } else {
    Object.assign(r, { status: 'denied', note: clean(a.note, 140), answeredAt: now() });
  }
  return r;
}

export function publicAction(state, a, { warden, ctx }) {
  if (a.action === 'duelRequest' || a.action === 'duelAnswer') return duelRequestAction(state, a, warden);
  if (a.action?.startsWith('duel')) { if (!warden) throw new Error('Warden PIN required.'); return duelAction(state, a); }
  switch (a.action) {
    case 'forstall': return forstallAction(state, a, { warden, ...(ctx || { list: [], tokens: [] }) });
    case 'roll': {
    ...
    case 'pc': return pcOp(state, a, warden);
    case 'initiative': {
      const pc = findPc(state, a.id);
      if (!pc) throw new Error('No such character.');
...
function pcOp(state, a, warden) {
  ...
  switch (a.op) {
    ...
    case 'installUpgrade': { // pp. 93–100: kitbash with Scrap or buy with $; needs a free slot, one of each type
      ...
      } else if (money(pc.wallet) < it.cost) throw new Error(`Costs $${it.cost} (Wallet has $${money(pc.wallet)}).`);
      const k = attachUpgrade(tgt, it, a.target); // free slot + one of each type, or it throws
      if (a.pay === 'scrap') pc.scrap = String(money(pc.scrap) - it.scrapCost);
      else pc.wallet = String(+(money(pc.wallet) - it.cost).toFixed(2));
      pc.updated = Date.now();
    ...
    case 'attack': { // p. 41: spend the weapon's Grit, roll its pool at that Range; damage = Hits − target's Defense (p. 43)
      ...
      log(state, { type: 'roll', who: pc.name, label: `${wname} → ${e.name} · ${RANGE_NAME[rng]}${rng === 'short' && (/melee/i.test(w.type || '') || /^melee-/.test(w.itemId || '')) ? ' (thrown)' : ''}${a.dist != null ? ` (${a.dist}″)` : ''}${aim ? ' · Aim' : ''}${ammo ? ` · ${ammo}` : ''}`, pool: poolLabel(pool), spur, dice: r.dice, hits, aces: r.dice.filter((d) => d.face === 'ace').length });
    ...
    case 'removeThing': { // empty a weapon/gear slot, or drop the horse/mech/Forstall
      ...
      let what = '';
      // the same thing leaves the inventory list too
      const gone = a.target === 'weapon' ? pc.weapons[k] : a.target === 'gear' ? pc.gear[k] : pc[a.target];
      const invIdx = (pc.items || []).findIndex((i) => (gone?.itemId && i.itemId === gone.itemId) || (a.target === 'forstall' && gone?.model && i.name === gone.model) || (a.target === 'horse' && gone?.breed && i.name === gone.breed));
      if (invIdx >= 0) pc.items.splice(invIdx, 1);
    ...
    case 'pick': { // picked from the sheet's store list: fill the whole section (the API looks the item up, custom items included)
      const it = a.item;
      if (!it) throw new Error('Unknown item.');
      const i = int(a.i, 0, 2);
      const before = a.kind === 'weapon' ? pc.weapons[i] : a.kind === 'gear' ? pc.gear[i] : pc[a.kind];
      const oldId = before?.itemId;
      if (a.kind === 'weapon') Object.assign(pc.weapons[i], weaponFields(it));
      else if (a.kind === 'gear') Object.assign(pc.gear[i], gearFields(it));
      else if (a.kind === 'forstall') Object.assign(pc.forstall ||= blankForstall(), forstallFields(it));
      else if (a.kind === 'mech') Object.assign(pc.mech ||= blankMech(), mechFields(it));
      else if (a.kind === 'horse') Object.assign(pc.horse ||= blankHorse(), horseFields(it));
      else throw new Error('Pick where it goes.');
      // the inventory mirrors the sheet: the replaced item leaves it, the new one joins it
      pc.items ||= [];
      if (oldId && oldId !== it.id) { const k = pc.items.findIndex((x) => x.itemId === oldId); if (k >= 0 && placedIn(pc, pc.items[k]) === null) pc.items.splice(k, 1); }
      if (!pc.items.some((x) => x.itemId === it.id && placedIn(pc, x) !== null) || a.kind === 'weapon' && pc.items.filter((x) => x.itemId === it.id).length < pc.weapons.filter((w) => w.itemId === it.id).length)
        pc.items.push({ uid: id(), itemId: it.id, name: it.name, cat: it.cat || '', sub: it.sub || '', qty: 1 });
      pc.updated = Date.now();
      return { name: it.name };
    }
    case 'dropItem': { // an inventory item is gone: remove it and take it off the sheet
      const i = (pc.items || []).findIndex((x) => x.uid === a.uid);
      if (i < 0) throw new Error('That item isn’t in their inventory.');
      const [it] = pc.items.splice(i, 1);
      const also = pc.items.some((x) => x.itemId === it.itemId) && it.cat !== 'Weapons' ? null : unequipItem(pc, it.itemId, it.name);
      pc.updated = Date.now();
      log(state, { type: 'event', text: `${pc.name} no longer has ${it.name}.` });
      return { name: it.name, also };
    }
    ...
    case 'die':
      ...
      if (state.combat.current === pc.id) advance(state, false);
      break;
    case 'revive': // the Warden's call, when the story allows it: back on their feet with at least 1 Health
      if (!warden) throw new Error('Only the Warden can bring a character back.');
      if (!pc.dead) throw new Error(`${pc.name} hasn’t fallen.`);
      pc.dead = false; pc.bleeding = null; pc.health = Math.max(pc.health, 1);
      log(state, { type: 'event', text: `${pc.name} is back among the living.` });
      break;
  ...
  switch (a.action) {
    ...
    case 'checkStart': { // Warden calls for a Skill roll at a difficulty (p. 12)
      ...
      if (a.diff === 'challenge') { // p. 13: both sides roll the same Skill; most Hits wins, a tie rolls again
        ...
        log(state, { type: 'event', text: `Challenge: ${[...who.map((pid) => findPc(state, pid).name), ...(npc ? [npc.name] : [])].join(' vs ')} — everyone rolls ${skill}${ck.note ? ` · ${ck.note}` : ''}.` });
        return ck;
      }
    ...
    case 'enemy': {
      ...
      switch (a.op) {
        ...
        case 'submerged': e.submerged = !e.submerged; log(state, { type: 'event', text: `${e.name} ${e.submerged ? 'dives under the water — out of reach of Forstalls' : 'surfaces'}.`, hidden: true }); break;
    ...
    case 'start': {
      ...
      // only the picked characters and enemies fight; the rest sit this one out
      if (Array.isArray(a.posse)) c.party = state.posse.filter((p) => a.posse.includes(p.id)).map((p) => p.id); else c.party = null;
      if (Array.isArray(a.enemies)) state.enemies.forEach((e) => { e.out = !a.enemies.includes(e.id); });
      // p. 40: whoever springs a surprise takes the first turn
      if (a.surprise !== undefined) c.surprise = a.surprise === 'posse' ? state.posse.filter((p) => !p.dead && inFight(state, p)).map((p) => p.id) : a.surprise === 'enemies' ? ['enemies'] : [];
      if (!state.posse.some((p) => !p.dead && inFight(state, p)) && !state.enemies.some((e) => !e.defeated && !e.out)) throw new Error('Pick at least one fighter.');
      state.posse.filter((p) => !p.dead && inFight(state, p) && !c.init[p.id]).forEach((p) => publicAction(state, { action: 'initiative', id: p.id }, { warden: true }));
      if (state.enemies.some((e) => !e.out) && !c.init.enemies) wardenCombat(state, { action: 'enemyInitiative' });
    ...
    case 'join': { // bring a character or an enemy into the fight that's already going
      const pc = findPc(state, a.id), e = findEnemy(state, a.id);
      if (!pc && !e) throw new Error('Pick who joins.');
      if (pc) {
        if (pc.dead) throw new Error(`${pc.name} is dead.`);
        if (c.party && !c.party.includes(pc.id)) c.party.push(pc.id);
        if (!c.init[pc.id]) publicAction(state, { action: 'initiative', id: pc.id }, { warden: true }); // rolls Finesse and re-orders
        else if (c.active) orderSlots(state);
        pc.grit = GRIT + (pc.gritBonus || 0);
      } else {
        e.out = false;
        if (c.active && !c.slots.includes('enemies')) { if (!c.init.enemies) wardenCombat(state, { action: 'enemyInitiative' }); orderSlots(state); }
      }
      const who = pc || e;
      log(state, { type: 'event', text: `${who.name} joins the fight!` });
      return { name: who.name };
    }
    case 'leave': { // step out of the fight (fled, knocked away, never there)
      const pc = findPc(state, a.id), e = findEnemy(state, a.id);
      if (!pc && !e) throw new Error('Pick who leaves.');
      if (pc) { c.party = (c.party || state.posse.map((p) => p.id)).filter((id) => id !== pc.id); c.slots = c.slots.filter((s) => s !== pc.id); }
      else { e.out = true; if (!state.enemies.some((x) => !x.out)) c.slots = c.slots.filter((s) => s !== 'enemies'); }
      const who = pc || e;
      if (c.current === who.id) advance(state, false);
      log(state, { type: 'event', text: `${who.name} is out of the fight.` });
      return { name: who.name };
    }
    case 'setTurn': if (c.active && turnList(state).includes(a.key)) startTurn(state, a.key); return;
    case 'end':
      c.active = false; c.round = 0; c.slots = []; c.current = null; c.init = {}; c.surprise = []; c.tiebreak = {}; state.emp = null; state.scanRound = null; state.fsOperator = {}; c.party = null;
      state.enemies.forEach((e) => { delete e.out; });
      state.undoStack = [];
...
// ---------- Forstalls (pp. 81–87): Sweep, Burst, Edison's Rule 1 ----------
// A placed Forstall's operator: a living character whose token is within Arm's Reach (1″) of it.
export function operatorOf(state, f, pcId, tokens) {
  const p = findPc(state, pcId);
  if (!p || p.dead || p.bleeding || p.statuses?.Unconscious) throw new Error('They can’t work a Forstall right now.');
  const t = (tokens || []).find((x) => x.kind === 'pc' && x.ref === p.id);
  if (!t || !f.pos || hexDist(f.pos, t) > 1) throw new Error(`${p.name} has to be within Arm’s Reach (1″) of the ${f.name} to work it.`);
  return p;
}
// p. 84: a Forstall has one operator at a time — the first character to work a placed one this round
export function claimOperator(state, f, p) {
  const o = state.fsOperator?.[f.key];
  if (o && o.round === state.combat.round && o.pc !== p.id) throw new Error(`${o.name} is working the ${f.name} this round.`);
  (state.fsOperator ||= {})[f.key] = { round: state.combat.round, pc: p.id, name: p.name };
}
// ctx = { list: fields(battle, combat), tokens: battle.tokens, cave } — built by the API from the battle map.
function forstallAction(state, a, { warden, list, tokens, cave }) {
  const f = list.find((x) => x.key === a.key);
  if (!f && a.op !== 'edison' && a.op !== 'emp') throw new Error('That Forstall isn’t in play.');
  const c = state.combat;
  // owner: the character whose sheet it's on (their charges, crystals, Abilities).
  // A Forstall the Warden placed on the map can be worked by any character within Arm's Reach (1″) of it.
  const owner = f?.owner ? findPc(state, f.owner) : null;
  const op = f && !f.owner && a.op !== 'emp' && a.pc ? operatorOf(state, f, String(a.pc), tokens) : null;
  if (f && !f.owner && !warden && !op && a.op !== 'emp') throw new Error('Stand within Arm’s Reach (1″) of it to work it.');
  const pc = owner || op; // who's working it: pays the Grit, rolls, gets the note
  // In a fight a character works a Forstall on their own turn (the Warden can always step in)
  const onTurn = pc && c.active && c.current === pc.id;
  if (pc && c.active && !onTurn && !warden && a.op !== 'off') throw new Error(`Work the Forstall on ${pc.name}’s own turn.`);
  if (op && c.active) claimOperator(state, f, op);
  state.sweeps ||= {};
  switch (a.op) {
    case 'sweep': {
      if (owner) {
        const ch = Number(owner.forstall.charges) || 0;
        if (ch < 1) throw new Error(`${f.name} is out of battery charges — a Town Rest recharges it.`);
      }
      const clash = list.filter((o) => o.key !== f.key && o.sweep && touching(o, f));
      if (clash.length && !a.force) throw new Error(`EDISON: ${f.name} would be in Range of ${clash.map((o) => o.name).join(' and ')}, already Sweeping. Edison’s Rule 1: the waves cross, everyone within Short Range of either Forstall is Electrocuted [6], and both batteries melt.`);
      if (onTurn) { spendGrit(state, pc, f.grit, 'Sweeping'); }
      if (owner) owner.forstall.charges = Math.max(0, (Number(owner.forstall.charges) || 0) - 1);
      if (clash.length) { edison(state, list, tokens, [f, ...clash]); return { melted: true }; }
      const pool = parsePool(f.pool);
      if (cave) { if (pool.black) pool.black -= 1; else if (pool.gold) pool.gold -= 1; } // Rule 2: waves echo in caves
      const spur = pc ? hasSpur(pc, 'Forstalls') : false;
      const r = pool.black + pool.gold ? rollPool(pool.black, pool.gold, spur) : { dice: [], hits: 0, aces: 0 };
      // Mechanic's Forstall Efficiency (2/day): turn one Hit into an Ace after the roll
      let eff = '';
      if (a.efficiency && owner && (owner.abilities || []).includes('Forstall Efficiency')) {
        const used = owner.abilityUses?.['Forstall Efficiency'] || 0;
        const d = r.dice.find((x) => x.face === 'hit');
        if (used >= 2) throw new Error('Forstall Efficiency is used up for today (2/day).');
        if (d) { d.face = 'ace'; r.hits += 1; r.aces = (r.aces || 0) + 1; owner.abilityUses = { ...(owner.abilityUses || {}), 'Forstall Efficiency': used + 1 }; eff = ' + Forstall Efficiency'; }
      }
      const again = !!state.sweeps[f.key];
      state.sweeps[f.key] = { hits: r.hits, pool: poolLabel(pool), by: pc?.name || 'Warden', at: now() };
      const label = `${f.name} — ${again ? 'readjusting the Sweep' : 'Sweeping'}${cave ? ' (cave, −1 die)' : ''}${eff}`;
      log(state, { type: 'roll', who: pc?.name || f.name, label, pool: poolLabel(pool), spur, dice: r.dice, hits: r.hits, aces: r.aces });
      if (onTurn) note(pc, `${again ? 'readjusted' : 'switched on'} ${f.name} → Sweep ${r.hits}`, f.grit);
      return { dice: r.dice, hits: r.hits, pool: poolLabel(pool), label, charges: owner ? owner.forstall.charges : null };
    }
    case 'off':
      if (!state.sweeps[f.key]) return;
      delete state.sweeps[f.key];
      log(state, { type: 'event', text: `${f.name} stops Sweeping.` });
      return;
    case 'burst': {
      const e = findEnemy(state, a.enemy);
      if (!e || e.defeated) throw new Error('Pick a monster that’s still standing.');
      if (!f.fuse) throw new Error(`${f.name} needs the Crystal Burst Fuse upgrade to Burst.`);
      if (f.jammed) throw new Error(`${f.name} is scrambled by a Natural EMP — no Bursting for a round.`);
      if (!f.known.has(e.profile)) throw new Error(`The ${e.profile || e.name}’s frequency isn’t programmed into ${f.name}.`);
      const tok = tokens.find((t) => t.kind === 'enemy' && t.ref === e.id);
      if (!f.pos || !tok) throw new Error('Both the Forstall and the monster need to be on the Battle Map.');
      if (!inRange(f, tok)) throw new Error(`${e.name} is ${hexDist(f.pos, tok)}″ away — outside ${f.name}’s ${f.range} Range.`);
      let gi = -1;
      if (owner) {
        gi = owner.gear.findIndex((g) => /crystal/i.test(g.item || ''));
        if (gi < 0) throw new Error(`${owner.name} has no Refined Crystal in their gear — each Burst shatters one.`);
      }
      const near = list.filter((o) => o.key !== f.key && o.sweep && o.pos && hexDist(o.pos, f.pos) <= 18);
      if (near.length && !a.force) throw new Error(`EDISON: ${near.map((o) => o.name).join(' and ')} ${near.length > 1 ? 'are' : 'is'} Sweeping within Long Range. Edison’s Rule 1: Bursting now Electrocutes [6] everyone within Short Range of either Forstall and melts the batteries.`);
      if (owner) owner.gear[gi] = blankGear();
      e.defeated = true; e.fled = true;
      log(state, { type: 'event', text: `${f.name} Bursts on the ${e.profile || e.name}’s frequency! The crystal shatters and ${e.name} flees for at least two hours.` });
      if (onTurn) note(pc, `Burst ${f.name} → ${e.name} fled`, 0);
      if (near.length) edison(state, list, tokens, [f, ...near]);
      if (c.current === e.id) advance(state, false);
      return { fled: e.name };
    }
    case 'emp': { // Southern Death Worm, Natural EMP: twice a day, every Forstall in Long Range stops; no Scan or Burst for a round
      if (!warden) throw new Error('Warden PIN required.');
      const e = findEnemy(state, a.enemy);
      if (!e || !(profileFor(e.profile)?.features || []).some((t) => /^Natural EMP/.test(t))) throw new Error('Only a monster with Natural EMP can do that.');
      if ((e.empUses || 0) >= 2) throw new Error(`${e.name} has used Natural EMP twice today.`);
      const tok = tokens.find((t) => t.kind === 'enemy' && t.ref === e.id);
      if (!tok) throw new Error('Put the monster on the Battle Map first.');
      const hit = list.filter((x) => x.pos && hexDist(x.pos, tok) <= 18);
      hit.forEach((x) => delete state.sweeps[x.key]);
      e.empUses = (e.empUses || 0) + 1;
      state.emp = { by: e.id, name: e.name, keys: hit.map((x) => x.key) };
      log(state, { type: 'event', text: `Static crackles — ${e.name} lets off a Natural EMP!${hit.length ? ` ${hit.map((x) => x.name).join(', ')} stop${hit.length === 1 ? 's' : ''} Sweeping, and can’t Scan or Burst until its next turn.` : ' No Forstall is close enough to feel it.'}` });
      return { hit: hit.map((x) => x.name) };
    }
    case 'edison': { // the Warden applies Rule 1 by hand
      if (!warden) throw new Error('Warden PIN required.');
      const fs = (a.keys || []).map((k) => list.find((x) => x.key === k)).filter(Boolean);
      if (fs.length < 2) throw new Error('Pick the two Forstalls.');
      edison(state, list, tokens, fs);
      return;
    }
    default: throw new Error('Unknown Forstall action.');
  }
}
// Rule 1: Electrocuted [6] for everyone within Short Range of any of these Forstalls; their batteries melt and the Sweeps stop.
function edison(state, list, tokens, fs) {
  const hit = new Set();
  for (const t of tokens) {
    if (!t.ref || !fs.some((f) => f.pos && hexDist(f.pos, t) <= 6)) continue;
    const who = t.kind === 'pc' ? findPc(state, t.ref) : t.kind === 'enemy' ? findEnemy(state, t.ref) : null;
    if (!who || who.dead || who.defeated || hit.has(who.id)) continue;
    if (t.kind === 'enemy' && /Electrocuted \[Immune\]/i.test(profileFor(who.profile)?.tolerances || '')) continue;
    hit.add(who.id);
    setStatus(who, 'Electrocuted', 6, who.size || 'Human');
  }
  for (const f of fs) {
    delete state.sweeps?.[f.key];
    const pc = f.owner && findPc(state, f.owner);
    if (pc?.forstall) pc.forstall.charges = 0;
  }
  const names = [...hit].map((id) => (findPc(state, id) || findEnemy(state, id)).name);
  log(state, { type: 'event', text: `Edison’s Rule 1! ${fs.map((f) => f.name).join(' and ')} cross waves — sparks fly, the batteries melt${names.length ? `, and ${names.join(', ')} ${names.length > 1 ? 'are' : 'is'} Electrocuted [6]` : ''}.` });
}
// A monster starts its turn in, or walks into, a Sweeping Forstall's Range (p. 82). Returns what happened.
export function sweepHit(state, list, tokens, enemyId, why) {
  const e = findEnemy(state, enemyId);
  const tok = e && tokens.find((t) => t.kind === 'enemy' && t.ref === e.id);
  if (!e || e.defeated || !tok) return null;
  const b = bestSweep(list, e, tok);
  if (!b) return null;
  e.grit = Math.max(0, (e.grit || 0) - b.loss);
  const who = tok.hidden ? 'Something out there' : `The ${e.name}`;
  const text = why === 'enter'
    ? `${who} cries out in distress as it comes into ${b.f.name}’s Range!`
    : b.loss ? `${who} writhes in ${b.f.name}’s hum.` : `${who} shrugs off ${b.f.name}’s hum.`;
  log(state, { type: 'event', text, secret: `${e.name}: −${b.loss} Grit (${lossText(b)}${b.why ? ` · ${b.why}` : ''}) → ${e.grit} Grit.` });
  if (state.combat.active && state.combat.current === e.id) note(e, `Forstall hum −${b.loss} Grit`, 0);
  return { loss: b.loss, grit: e.grit };
}

// ---------- "Needs you": everything waiting on the Warden, from every page (Warden strip) ----------
export function wardenNeeds(state, { shop, battle, edison, whispers }) {
  const out = [], c = state.combat, name = (id) => findPc(state, id)?.name || '?';
  for (const w of (whispers?.list || []).filter((x) => !x.read)) out.push({ urgent: true, href: '/run', text: `${w.name} whispered: “${w.text.length > 70 ? `${w.text.slice(0, 70)}…` : w.text}”` });
  for (const r of (shop?.requests || []).filter((x) => x.status === 'pending')) out.push({ urgent: true, href: '/store', store: r.id, text: `${r.pcName} wants to ${r.kind} ${r.qty > 1 ? `${r.qty}× ` : ''}${r.name} ($${Number(r.price).toFixed(2)})` });
  for (const ck of state.checks || []) {
    const waiting = ck.who.filter((pid) => !ck.rolls[pid]).map(name);
    if (ck.kind === 'challenge') {
      out.push(ck.winner ? { urgent: true, href: '/run#grp-rolls', closeCheck: ck.id, text: `${ck.skill} Challenge is in: ${ck.winner} wins. Close it?` }
        : { urgent: false, href: '/run#grp-rolls', text: `${ck.skill} Challenge — waiting on ${waiting.join(', ') || 'a reroll'}` });
      continue;
    }
    if (waiting.length) { out.push({ urgent: false, href: '/run#grp-rolls', text: `${ck.skill} check — waiting on ${waiting.join(', ')}` }); continue; }
    // everyone's rolled: say how it went and offer to close it (the posse can still Help until then)
    const res = ck.who.map((pid) => { const o = checkOutcome(state, ck, pid); return `${name(pid)} ${o.ok ? 'made it' : 'missed'} (${o.total}/${ck.target})`; });
    out.push({ urgent: true, href: '/run#grp-rolls', closeCheck: ck.id, text: `${ck.skill} check is in: ${res.join(', ')}. Close it?` });
  }
  for (const r of (state.duelRequests || []).filter((x) => x.status === 'pending')) out.push({ urgent: true, href: '/run', text: `${r.name} challenges ${r.npcName} to a Duel` });
  for (const p of state.posse.filter((x) => x.hold?.triggeredBy)) out.push({ urgent: true, href: '/battle', text: `${p.name}’s prepared ${p.hold.label} was set off` });
  for (const p of state.posse.filter((x) => x.bleeding && !x.dead)) out.push({ urgent: true, href: `/posse#${p.id}`, text: `${p.name} is Bleeding Out` });
  for (const [a, b] of edison || []) out.push({ urgent: true, href: '/battle', text: `Edison’s Rule 1: ${a} and ${b} are Sweeping in each other’s Range` });
  if (c.active && c.current) {
    const e = findEnemy(state, c.current);
    out.push(e ? { urgent: true, href: '/battle', text: `${e.name}’s turn — your move (Round ${c.round})` } : { urgent: false, href: '/battle', text: `${name(c.current)}’s turn (Round ${c.round})` });
  }
  return { items: out, count: out.filter((x) => x.urgent).length };
}
  ...
  return {
    ...
    hud: {
      ...
      checks: (state.checks || []).filter((ck) => !ck.winner).map((ck) => ({ id: ck.id, kind: ck.kind || 'check', skill: ck.skill, diff: ck.diff, target: ck.target, note: ck.note,
        round: ck.round || 1, who: ck.who.map((pid) => ({ id: pid, name: name(pid), rolled: !!ck.rolls[pid] })), vs: ck.npc?.name || '', helped: Object.keys(ck.helps || {}) })),
      duel: state.duel ? { ...state.duel, next: DUEL_STEPS[state.duel.step] || null } : null,
      duelReqs: (state.duelRequests || []).slice(0, 10).map(({ id, pc, name, npcName, reason, status, note, tough, at }) => ({ id, pc, name, npcName, reason, status, note, tough, at })),
      trades: (state.trades || []).filter((t) => t.status === 'pending' || Date.now() - (t.answeredAt || t.at) < 86400000).slice(0, 10),
...
export function playerCombatView(state) {
  ...
  return {
    v: state.v,
    scanRound: state.scanRound || null, // who worked a Forstall's Scan this round (p. 84: one Scanner per round)
    fsOperator: state.combat.active ? Object.fromEntries(Object.entries(state.fsOperator || {}).filter(([, o]) => o.round === state.combat.round)) : {},
    stash: { money: state.stash?.money || 0, items: (state.stash?.items || []).map(({ id: i, name, qty, where }) => ({ id: i, name, qty, where })) },
    // each inventory row says where it sits on the sheet (the sheet sections are the record; the list mirrors them)
    posse: state.posse.map((p) => ({ ...p, items: (p.items || []).map((it) => ({ ...it, placed: placedIn(p, it) })) })),
    enemies: state.enemies.map((e) => ({
      id: e.id, name: e.name, profile: e.profile && sweepTolerance(e).monster ? e.profile : null, // monsters' kind: what a Forstall Scans
      size: e.size, statuses: e.statuses, defeated: e.defeated, out: !!e.out, frenzied: e.frenzied.length > 0, grit: e.grit, speed: e.speed, turnLog: e.turnLog || [], crippled: !!e.crippled,
