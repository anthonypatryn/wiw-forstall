...
const UNDO_ACTIONS = new Set(['enemyAttack', 'next', 'roll', 'enemy', 'checkRoll', 'forstall', 'clock']);
...
export function pushUndo(state, label = '', extra = {}) {
  const snap = JSON.parse(JSON.stringify({ posse: state.posse, enemies: state.enemies, combat: state.combat, duel: state.duel ?? null, checks: state.checks ?? [], sweeps: state.sweeps ?? {}, clock: state.clock ?? { t: 8 } }));
...
function wardenCombat(state, a) {
  ...
  switch (a.action) {
    ...
    case 'clock': { // the Warden moves the game clock on
      const cl = clockOf(state);
      const hours = a.morning ? (24 - (cl.t % 24)) + 8 - (cl.t % 24 < 8 ? 24 : 0) : int(a.hours, 1, 72);
      return passTime(state, Math.max(1, hours));
    }
    case 'setTurn': if (c.active && turnList(state).includes(a.key)) startTurn(state, a.key); return;
    case 'end':
      c.active = false; c.round = 0; c.slots = []; c.current = null; c.init = {}; c.surprise = []; c.tiebreak = {}; state.emp = null; state.scanRound = null;
      state.undoStack = [];
...
function forstallAction(state, a, { warden, list, tokens, cave }) {
  ...
  switch (a.op) {
    case 'sweep': {
      ...
      // a character's battery charge lasts 2+ hours of game time; the Warden's Forstalls run off the town's power
      const until = pc ? (again && state.sweeps[f.key].until != null ? state.sweeps[f.key].until : clockOf(state).t + sweepHours(pc.forstall)) : null;
      state.sweeps[f.key] = { hits: r.hits, pool: poolLabel(pool), by: pc?.name || 'Warden', at: now(), until, untilLabel: until != null ? clockLabel(until) : '' };
...
// ---------- the game clock: Sweeps burn battery charges, 2/day abilities come back each morning ----------
export const clockOf = (state) => (state.clock ||= { t: 8 }); // hours since Day 1, midnight — starts 8 AM
export const clockLabel = (t) => { const h = t % 24; return `Day ${Math.floor(t / 24) + 1} · ${h % 12 || 12} ${h < 12 ? 'AM' : 'PM'}`; };
// p. 82: one battery charge runs a Sweep for 2 hours, longer with the Sweeping Duration upgrade (4/6/8 hours)
export function sweepHours(f) {
  const up = (f?.upgrades || []).map((u) => Number(String(u).match(/\((\d+) hours?\)/)?.[1] || 0));
  return Math.max(2, Number(String(f?.duration || '').replace(/\D/g, '')) || 2, ...up);
}
export function passTime(state, hours) {
  const c = clockOf(state), from = c.t, notes = [];
  c.t += hours;
  for (const [key, sw] of Object.entries(state.sweeps || {})) {
    if (!key.startsWith('pc:') || sw.until == null) continue;
    const pc = findPc(state, key.slice(3));
    if (!pc?.forstall?.model) { delete state.sweeps[key]; continue; }
    const dur = sweepHours(pc.forstall);
    while (sw.until <= c.t) {
      const left = Number(pc.forstall.charges) || 0;
      if (left > 0) { pc.forstall.charges = left - 1; sw.until += dur; notes.push(`${pc.name}’s Forstall uses another battery charge (${left - 1} left)`); }
      else { delete state.sweeps[key]; notes.push(`${pc.name}’s ${pc.forstall.model} runs out of charge and stops Sweeping`); break; }
    }
  }
  if (Math.floor(c.t / 24) > Math.floor(from / 24)) {
    state.posse.forEach((p) => { p.abilityUses = {}; p.horseGrit = 0; });
    state.enemies.forEach((e) => { e.empUses = 0; });
    notes.push('A new day — 2/day abilities are ready again');
  }
  log(state, { type: 'event', text: `Time passes: it’s ${clockLabel(c.t)}.${notes.length ? ` ${notes.join('. ')}.` : ''}` });
  return { clock: clockLabel(c.t), notes };
}
...
export function wardenNeeds(state, { shop, battle, edison }) {
  ...
  for (const r of (shop?.requests || []).filter((x) => x.status === 'pending')) out.push({ urgent: true, href: '/store', text: `${r.pcName} wants to ${r.kind} ${r.qty > 1 ? `${r.qty}× ` : ''}${r.name} ($${Number(r.price).toFixed(2)})` });
  ...
  return { items: out, count: out.filter((x) => x.urgent).length, clock: clockLabel(clockOf(state).t) };
}
...
export function logView(state) {
  ...
  return {
    ...
    hud: {
      clock: clockLabel(clockOf(state).t),
...
export function playerCombatView(state) {
  ...
  return {
    ...
    clock: { t: clockOf(state).t, label: clockLabel(clockOf(state).t) },
    sweeps: state.sweeps || {},
