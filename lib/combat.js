...
export function pushUndo(state, label = '', extra = {}) {
  const snap = JSON.parse(JSON.stringify({ posse: state.posse, enemies: state.enemies, combat: state.combat, duel: state.duel ?? null, checks: state.checks ?? [], sweeps: state.sweeps ?? {} }));
