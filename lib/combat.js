...
import { newSheet, setSheetField, toggleSheetList, hasSpur, giveStartingWeapons, weaponFields, gearFields, tierRanged, tierMelee, upgradeFits, upgradeType, mechState, TIERS, ACHIEVEMENTS, EXTRA_CATS, STARTING_WEAPONS, TALENTS, REPUTATION, MECH_STATES, PACKS, KEEPSAKES } from './sheets.js';
...
function pcOp(state, a) {
  ...
  switch (a.op) {
    ...
    case 'attack': { // p. 41: spend the weapon's Grit, roll its pool at that Range; damage = Hits − target's Defense (p. 43)
      ...
      const aim = !!a.aim;
      if (aim && pc.aimed) throw new Error('Aim is once per turn.');
      if (aim && pc.statuses?.Electrocuted) throw new Error('Electrocuted — you can’t Aim.');
      const cost = (parseInt(String(w.grit).split('|')[0], 10) || 0) + (aim ? 1 : 0);
      spendGrit(state, pc, cost, 'That attack');
    ...
    case 'checkRoll': { // answer the Warden's Skill check — or Help someone with half your dice (p. 13)
      ...
      return { ...entry, helping, outcome: helping ? null : checkOutcome(state, ck, pc.id), target: ck.target };
    }
...
function wardenCombat(state, a) {
  ...
  switch (a.action) {
    ...
    case 'enemyAttack': { // Warden: roll an enemy attack at a posse member; their Defense + Cover + Dodge soak it (p. 43)
      ...
      if (state.combat.active && at.grit) e.grit = Math.max(0, (e.grit || 0) - at.grit);
...
export const META = { difficulty: DIFFICULTY, achievements: ACHIEVEMENTS, tiers: TIERS, packs: PACKS, keepsakes: KEEPSAKES, statuses: STATUSES, capture: CAPTURE, trades: TRADES, talents: TALENTS, reputationLevels: REPUTATION, mechStates: MECH_STATES, skills: SKILLS };
