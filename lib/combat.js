...
import { newSheet, setSheetField, toggleSheetList, hasSpur, giveStartingWeapons, weaponFields, gearFields, tierRanged, tierMelee, TIERS, EXTRA_CATS, STARTING_WEAPONS, TALENTS, REPUTATION, MECH_STATES, PACKS, KEEPSAKES } from './sheets.js';
...
function pcOp(state, a) {
  ...
  switch (a.op) {
    ...
    case 'tierKit': { // Starting at Higher Prestige (p. 33): weapons, Scrap, Wallet, extra items, Prestige to spend
      ...
      // extra items go into Other items under their own header (replacing an earlier list)
      const inv = String(pc.inventory || '').replace(/(^|\n)— Starting extras —\n(?:[^\n]+\n?)*/, '$1').trim();
      pc.inventory = (extras.length ? `${inv ? inv + '\n\n' : ''}— Starting extras —\n${extras.map((i) => i.name).join('\n')}` : inv).slice(0, 3000);
      pc.tier = t.name; pc.tierApplied = t.name; pc.tierKit = [ranged.id, melee.id];
...
export const META = { tiers: TIERS, packs: PACKS, keepsakes: KEEPSAKES, statuses: STATUSES, capture: CAPTURE, trades: TRADES, talents: TALENTS, reputationLevels: REPUTATION, mechStates: MECH_STATES, skills: SKILLS };
...
export function wardenCombatView(state) {
  const profiles = Object.fromEntries(state.enemies.filter((e) => e.profile).map((e) => [e.profile, profileFor(e.profile)]));
  return {
