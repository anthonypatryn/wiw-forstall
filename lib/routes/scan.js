...
async function combatScan(state, body) {
  ...
  const c = combat?.combat, fight = !!c?.active; // out of a fight: free, any time (Range and the one guess still apply)
  const pc = combat?.posse?.find((p) => p.id === String(body.pc || ''));
  if (!pc) throw new Error('Pick who’s Scanning. (Tap “This is me” on your sheet first.)');
  if (fight && c.current !== pc.id) throw new Error(`Scan on ${pc.name}’s own turn.`);
  ...
  if (!f.owner && fight) claimOperator(combat, f, pc); // one operator per placed Forstall per round
  ...
  const sr = fight ? combat.scanRound : null;
  ...
  const cost = fight ? scanCost(state) : 0;
