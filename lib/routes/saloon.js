...
function decorate(view, state, combat, pcId) {
  ...
  // every seat's Skill pools, so a move shows the real matchup (a character's come from their sheet, Poisoned counted)
  t.seats = t.seats.map((s) => {
    if (s.kind === 'npc') return { ...s, skills: Object.fromEntries(SKILLS.map((k) => [k, npcs[s.key]?.[k] || '2B'])) };
    const sp = combat.posse.find((p) => p.id === s.pc);
    return sp ? { ...s, skills: Object.fromEntries(SKILLS.map((k) => [k.toLowerCase(), poolLabel(poolFor(sp, k))])) } : s;
  });
