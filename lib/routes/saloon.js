...
const KEY = 'saloon';
const GAME = { poker: 'Poker', faro: 'Faro', liars: 'Liar’s Dice', blackjack: 'Blackjack' };
const SKILLS = ['charm', 'finesse', 'intuition'];
...
function decorate(view, state, combat, pcId) {
  ...
  if (pc) t.me = { poisoned: !!pc.statuses?.Poisoned, skills: Object.fromEntries(SKILLS.map((k) => [k, poolLabel(poolFor(pc, k))])) };
  return view;
}
  ...
  try {
    ...
    const ctx = {
      ...
      roll: (seat, skill, pool) => {
        ...
        const p = pc ? poolFor(pc, skill) : parsePool(pool);
