...
const NS = new Set(['pc', 'handout', 'wanted', 'clue']);
...
export default async function handler(req, res) {
  try {
    ...
    if (ns !== 'pc' && !warden) return send(res, 401, { error: 'Warden PIN required.' });
    const parse = (s, size) => {
    ...
    // a clue's picture is noted on the clue
    if (ns === 'clue') {
      const js = (await load('journal')) || { v: 0, quests: [], clues: [] };
      const c = js.clues.find((x) => x.id === id);
      if (c) { c.img = v; js.v = (js.v || 0) + 1; await save(js, 'journal'); }
    }
