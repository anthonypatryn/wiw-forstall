...
import { newLiarsGame, liarsBid, liarsCall, liarsPeek, liarsStare, liarsRefund, liarsView } from './liars.js';
import { newContest, pour, drink, spittoon, needle, drinkRefund, drinkBusy, drinkView } from './drinking.js';
import { newBjRound, bjBet, bjDeal, bjMove, bjCount, bjShiner, bjRefund, bjResume, bjBusy, bjView } from './blackjack.js';
import { newFaroDeal, faroBet, faroCall, faroTurn, faroWatch, faroRefund, faroView } from './faro.js';
import { clean, int, cents, money } from './util.js';
import { seatOf, sharpest } from './saloon-common.js';
export { money }; // lib/routes/saloon.js imports it from here

export const freshSaloon = () => ({ v: 0, table: null });
const STYLES = ['tight', 'loose', 'bluffer'];
...
export function saloonAction(state, a, ctx) {
  ...
  switch (a.action) {
    case 'open': { // the Warden opens a table
      ...
      const game = ['faro', 'liars', 'blackjack', 'drinking'].includes(a.game) ? a.game : 'poker';
      if (game === 'faro' || game === 'blackjack') npcs.splice(1); // one dealer banks faro and blackjack // one dealer banks the faro game
      state.table = {
        id: crypto.randomUUID().slice(0, 8), game, crooked: game === 'faro' && !!a.crooked, status: 'open', where: clean(a.where, 60) || 'the saloon',
        stakes: { ante: money(a.ante) || 1, bet: money(a.bet) || 2 }, hooks: { tell: a.tell !== false, bluff: a.bluff !== false, palm: a.palm !== false, peek: a.peek !== false, stare: a.stare !== false, count: a.count !== false, shiner: a.shiner !== false, spittoon: a.spittoon !== false, needle: a.needle !== false },
        invite: (Array.isArray(a.invite) ? a.invite : []).map((x) => clean(x, 12)), seats: npcs, dealer: 0, hand: null, handsPlayed: 0, at: Date.now(),
      };
      if (game === 'drinking') log(`The bartender lines up glasses at ${state.table.where}: a drinking contest, $${state.table.stakes.ante.toFixed(2)} to get in, last one standing takes the pot.`);
      else if (game === 'blackjack') log(`${npcs[0].name} deals blackjack at ${state.table.where}: bets from $${state.table.stakes.ante.toFixed(2)}, blackjack pays 3 to 2.`);
      else if (game === 'liars') log(`A game of Liar’s Dice starts up at ${state.table.where}: $${state.table.stakes.ante.toFixed(2)} a head, winner takes the pot.`);
      else log(game === 'faro' ? `${npcs[0].name} opens a faro bank at ${state.table.where}. Bets from $${state.table.stakes.ante.toFixed(2)} a card.` : `A card table opens at ${state.table.where}: five-card draw, $${state.table.stakes.ante.toFixed(2)} ante. Pull up a chair.`);
      return state.table;
    }
    case 'join': {
    ...
    case 'leave': {
      ...
      if (t.game === 'drinking' && drinkBusy(t, s)) throw new Error('You’re still upright — drink or pass out first.');
      if (t.game === 'blackjack' && bjBusy(t, s)) throw new Error('Finish this hand first.');
      if (t.game === 'blackjack') { bjRefund(t, s, ctx); bjResume(t, ctx, log); }
      if (t.game === 'faro') faroRefund(t, s, ctx);
      if (t.game === 'liars' && t.liars && !t.liars.over && t.liars.counts[s.key] > 0) throw new Error('You still have dice in this game — play it out.');
    ...
    case 'deal': { // the Warden (or any seated player) deals the next hand
      ...
      if (!warden) me();
      if (t.game === 'liars') {
        if (t.liars && !t.liars.over) throw new Error('This game isn’t finished yet.');
        newLiarsGame(t, ctx, log); t.status = 'playing';
        return;
      }
      if (t.game === 'drinking') {
        if (t.drink && !t.drink.over) throw new Error('This contest isn’t over yet.');
        newContest(t, ctx, log); t.status = 'playing';
        return;
      }
      if (t.game === 'blackjack') {
        if (t.bj && t.bj.phase !== 'done') throw new Error('This round isn’t finished yet.');
        newBjRound(t, ctx); t.status = 'playing';
        return;
      }
      if (t.game === 'faro') {
        if (t.faro && !t.faro.over) throw new Error('This deal isn’t finished yet.');
        newFaroDeal(t, ctx); t.status = 'playing';
        return;
      }
    ...
    case 'tell': { // Intuition vs the NPC's Charm: win and you see one of their cards
      ...
      if (!target || target.key === s.key || !h.order.includes(target.key) || h.folded[target.key]) throw new Error('Pick someone still in the hand.');
    ...
    case 'bluff': { // Charm vs each NPC's Intuition: the ones you beat fold unless they hold two pair or better
      ...
      const out = [];
      for (const n of inHand(t).filter((x) => x.key !== s.key)) {
        const won = challenge(s, n, 'charm', 'intuition', ctx);
        if (!won) continue;
        // an NPC folds unless it holds two pair or better; a player can't be made to fold — they're warned instead
        if (n.kind === 'npc') { if (rankHand(h.cards[n.key]).cat < 2) { n.bluffed = true; out.push(n.name); } }
        else { (h.warned ||= {})[n.key] = s.name; out.push(n.name); }
      }
    ...
    case 'palm': { // Finesse vs the sharpest NPC's Intuition, on your draw: swap one card for the better of two from the deck
      ...
      const eye = sharpest(inHand(t).filter((x) => x.key !== s.key), ctx);
    ...
    case 'liarsBid': { const s = me(); if (t.game !== 'liars') throw new Error('Not a dice table.'); return liarsBid(t, s, a, ctx, log); }
    case 'liarsCall': { const s = me(); if (t.game !== 'liars') throw new Error('Not a dice table.'); return liarsCall(t, s, ctx, log); }
    case 'liarsPeek': { const s = me(); if (t.game !== 'liars') throw new Error('Not a dice table.'); return liarsPeek(t, s, a, ctx, challenge); }
    case 'liarsStare': { const s = me(); if (t.game !== 'liars') throw new Error('Not a dice table.'); return liarsStare(t, s, ctx, challenge); }
    case 'drinkPour': { if (!warden) me(); if (t?.game !== 'drinking') throw new Error('Not a drinking contest.'); return pour(t, ctx, log, challenge); }
    case 'drink': { const s = me(); if (t.game !== 'drinking') throw new Error('Not a drinking contest.'); return drink(t, s, a, ctx, log); }
    case 'drinkSpit': { const s = me(); if (t.game !== 'drinking') throw new Error('Not a drinking contest.'); return spittoon(t, s, ctx, challenge); }
    case 'drinkNeedle': { const s = me(); if (t.game !== 'drinking') throw new Error('Not a drinking contest.'); return needle(t, s, a, ctx, challenge); }
    case 'bjBet': { const s = me(); if (t.game !== 'blackjack') throw new Error('Not a blackjack table.'); return bjBet(t, s, a, ctx, log); }
    case 'bjDeal': { if (!warden) me(); if (t?.game !== 'blackjack') throw new Error('Not a blackjack table.'); return bjDeal(t, ctx, log); }
    case 'bjMove': { const s = me(); if (t.game !== 'blackjack') throw new Error('Not a blackjack table.'); return bjMove(t, s, a.move, ctx, log); }
    case 'bjCount': { const s = me(); if (t.game !== 'blackjack') throw new Error('Not a blackjack table.'); return bjCount(t, s, ctx, challenge); }
    case 'bjShiner': { const s = me(); if (t.game !== 'blackjack') throw new Error('Not a blackjack table.'); return bjShiner(t, s, ctx, challenge, log); }
    case 'faroBet': { const s = me(); if (t.game !== 'faro') throw new Error('Not a faro table.'); return faroBet(t, s, a, ctx); }
    case 'faroCall': { const s = me(); if (t.game !== 'faro') throw new Error('Not a faro table.'); return faroCall(t, s, a, ctx); }
    case 'faroTurn': { if (!warden) me(); if (t?.game !== 'faro') throw new Error('Not a faro table.'); return faroTurn(t, ctx, log); }
    case 'faroWatch': { const s = me(); if (t.game !== 'faro') throw new Error('Not a faro table.'); return faroWatch(t, s, ctx, challenge, log); }
    case 'kick': {
      if (!warden) throw new Error('Warden PIN required.');
      if (!t) return;
      ...
      if (t.game === 'blackjack') { bjRefund(t, s, ctx); bjResume(t, ctx, log); }
      if (t.game === 'drinking' && drinkBusy(t, s)) drinkRefund(t, ctx);
      if (t.game === 'faro') faroRefund(t, s, ctx);
      if (t.game === 'liars' && t.liars && !t.liars.over && t.liars.counts[s.key] > 0) {
        if (t.liars.order.filter((k) => t.liars.counts[k] > 0).length <= 2) liarsRefund(t, ctx);
        else { t.liars.counts[s.key] = 0; if (t.liars.turn === s.key) { const i = t.liars.order.indexOf(s.key); t.liars.turn = t.liars.order.slice(i + 1).concat(t.liars.order.slice(0, i)).find((k) => t.liars.counts[k] > 0); } }
      }
      t.seats = t.seats.filter((x) => x !== s);
      return;
    }
    case 'close': {
      ...
      if (t.game === 'faro') faroRefund(t, null, ctx);
      if (t.game === 'liars') liarsRefund(t, ctx);
      if (t.game === 'blackjack') bjRefund(t, null, ctx);
      if (t.game === 'drinking') drinkRefund(t, ctx);
...
export function saloonView(state, { warden, pc }) {
  ...
  const hand = h && {
    ...
    warnedBy: mine ? h.warned?.[mine] || null : null,
  ...
  return {
    v: state.v,
    table: { faro: t.game === 'faro' ? faroView(t, { warden, mine }) : undefined, liars: t.game === 'liars' ? liarsView(t, { warden, mine }) : undefined, bj: t.game === 'blackjack' ? bjView(t, { warden, mine }) : undefined, drink: t.game === 'drinking' ? drinkView(t, { warden, mine }) : undefined, id: t.id, game: t.game, status: t.status, where: t.where, stakes: t.stakes, hooks: t.hooks, invite: t.invite, handsPlayed: t.handsPlayed, dealer: t.dealer,
