...
export function saloonAction(state, a, ctx) {
  ...
  switch (a.action) {
    case 'open': { // the Warden opens a table
      ...
      const game = ['faro', 'liars'].includes(a.game) ? a.game : 'poker';
      if (game === 'faro') npcs.splice(1); // one dealer banks the faro game
      state.table = {
        id: crypto.randomUUID().slice(0, 8), game, crooked: game === 'faro' && !!a.crooked, status: 'open', where: clean(a.where, 60) || 'the saloon',
        stakes: { ante: money(a.ante) || 1, bet: money(a.bet) || 2 }, hooks: { tell: a.tell !== false, bluff: a.bluff !== false, palm: a.palm !== false, peek: a.peek !== false, stare: a.stare !== false },
        invite: (Array.isArray(a.invite) ? a.invite : []).map((x) => clean(x, 12)), seats: npcs, dealer: 0, hand: null, handsPlayed: 0, at: Date.now(),
      };
      if (game === 'liars') log(`A game of Liar’s Dice starts up at ${state.table.where}: $${state.table.stakes.ante.toFixed(2)} a head, winner takes the pot.`);
...
export function saloonView(state, { warden, pc }) {
  ...
  return {
    v: state.v,
    table: { faro: t.game === 'faro' ? faroView(t, { warden, mine }) : undefined, liars: t.game === 'liars' ? liarsView(t, { warden, mine }) : undefined, id: t.id, game: t.game, status: t.status, where: t.where, stakes: t.stakes, hooks: t.hooks, invite: t.invite, handsPlayed: t.handsPlayed, dealer: t.dealer,
