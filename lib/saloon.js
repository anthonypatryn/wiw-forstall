...
export function saloonAction(state, a, ctx) {
  ...
  switch (a.action) {
    case 'open': { // the Warden opens a table
      ...
      const game = a.game === 'faro' ? 'faro' : 'poker';
      ...
      log(game === 'faro' ? `${npcs[0].name} opens a faro bank at ${state.table.where}. Bets from $${state.table.stakes.ante.toFixed(2)} a card.` : `A card table opens at ${state.table.where}: five-card draw, $${state.table.stakes.ante.toFixed(2)} ante. Pull up a chair.`);
      return state.table;
    }
    case 'join': {
...
export function saloonView(state, { warden, pc }) {
  ...
  return {
    v: state.v,
    table: { faro: t.game === 'faro' ? faroView(t, { warden, mine }) : undefined, id: t.id, game: t.game, status: t.status, where: t.where, stakes: t.stakes, hooks: t.hooks, invite: t.invite, handsPlayed: t.handsPlayed, dealer: t.dealer,
