...
export function saloonAction(state, a, ctx) {
  ...
  switch (a.action) {
    case 'open': { // the Warden opens a table
      ...
      const game = ['faro', 'liars', 'blackjack'].includes(a.game) ? a.game : 'poker';
      ...
      state.table = {
        ...
        stakes: { ante: money(a.ante) || 1, bet: money(a.bet) || 2 }, hooks: { tell: a.tell !== false, bluff: a.bluff !== false, palm: a.palm !== false, peek: a.peek !== false, stare: a.stare !== false, count: a.count !== false, shiner: a.shiner !== false },
        invite: (Array.isArray(a.invite) ? a.invite : []).map((x) => clean(x, 12)), seats: npcs, dealer: 0, hand: null, handsPlayed: 0, at: Date.now(),
      };
      if (game === 'blackjack') log(`${npcs[0].name} deals blackjack at ${state.table.where}: bets from $${state.table.stakes.ante.toFixed(2)}, blackjack pays 3 to 2.`);
...
export function saloonView(state, { warden, pc }) {
  ...
  return {
    v: state.v,
    table: { faro: t.game === 'faro' ? faroView(t, { warden, mine }) : undefined, liars: t.game === 'liars' ? liarsView(t, { warden, mine }) : undefined, bj: t.game === 'blackjack' ? bjView(t, { warden, mine }) : undefined, id: t.id, game: t.game, status: t.status, where: t.where, stakes: t.stakes, hooks: t.hooks, invite: t.invite, handsPlayed: t.handsPlayed, dealer: t.dealer,
