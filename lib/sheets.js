...
export const weaponSubOf = (w) => CATALOG.find((x) => x.id === w.itemId)?.sub || (/melee/i.test(w.type) ? 'Melee' : /mounted/i.test(w.type) ? 'Mounted' : Object.keys(SINGULAR).find((k) => w.type === k)) || '';
...
// Is this inventory entry already filled into its sheet section? (false = it belongs somewhere on the sheet but isn't there)
export function isPlaced(pc, entry) {
  const { cat, sub, itemId } = entry, items = pc.items || [];
  if (cat === 'Weapons') return pc.weapons.filter((w) => w.itemId === itemId).length >= items.filter((x) => x.itemId === itemId).length;
  if (cat === 'Forstalls' && !/crystal/i.test(entry.name)) return pc.forstall?.itemId === itemId || pc.forstall?.model === entry.name;
  if (cat === 'Mechs') return pc.mech?.itemId === itemId;
  if (sub === 'Horse Breeds' || sub === 'Legendary Steeds') return pc.horse?.itemId === itemId || pc.horse?.breed === entry.name;
  if (sub === 'Special Ammo & Arrows') return pc.weapons.some((w) => (w.ammo || []).some((a) => a.name === entry.name));
  if (cat === 'Upgrades') return sub === 'Trap Upgrades' || [...pc.weapons, pc.forstall, pc.mech].some((t) => (t?.upgradeIds || []).includes(itemId));
  if (cat === 'Gear' || cat === 'Traps' || cat === 'Forstalls') return pc.gear.some((g) => g.itemId === itemId);
  return true; // goods & services have no sheet section
}
