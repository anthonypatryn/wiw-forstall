...
const blankWeapon = () => ({ manufacturer: '', model: '', slots: '', grit: '', arms: '', short: '', long: '', upgrades: ['', '', '', ''], ammo: [{ name: '', rds: '' }, { name: '', rds: '' }] });
const blankGear = () => ({ item: '', type: '', grit: '', notes: '', uses: 0 });
...
export function newSheet(trade, name) {
  ...
  return {
    ...
    inventory: '', wallet: '', scrap: '', supplies: '',
...
const RULES = [
  ...
  [/^weapons\.[0-2]\.(manufacturer|model|slots|grit)$/, str(40)],
  [/^weapons\.[0-2]\.(arms|short|long)$/, pool],
  [/^weapons\.[0-2]\.upgrades\.[0-3]$/, str(60)],
  [/^weapons\.[0-2]\.ammo\.[01]\.(name|rds)$/, str(40)],
  [/^gear\.[0-2]\.(item|type|grit|notes)$/, str(80)],
  [/^gear\.[0-2]\.uses$/, num(0, 6)],
  [/^horse\.(name|breed|breakingPoint|maxHealth|health|breedAbility|disposition|appearance)$/, str(300)],
  [/^horse\.bond$/, oneOf(REPUTATION)],
  [/^forstall\.(model|slots|range|grit|duration)$/, str(40)],
  [/^forstall\.charges$/, num(0, 9)],
  [/^forstall\.(upgrades|kz)\.[0-3]$/, str(60)],
  [/^mech\.(class|slots|speed|maxHealth|health|supplies|cover)$/, str(40)],
  [/^mech\.defense$/, pool],
