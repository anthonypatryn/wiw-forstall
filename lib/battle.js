...
export function autoSync(state, combat) {
  ...
  if (combat.combat?.active) {
    ...
    (combat.posse || []).filter((p) => !p.dead && !have.has(p.id)).forEach((p) => { state.tokens.push({ id: crypto.randomUUID().slice(0, 8), kind: 'pc', ref: p.id, name: p.name, ...freeSpot(state, rows - 2), hidden: false }); changed = true; });
    (combat.enemies || []).filter((e) => !e.defeated && !have.has(e.id)).forEach((e) => { state.tokens.push({ id: crypto.randomUUID().slice(0, 8), kind: 'enemy', ref: e.id, name: e.name, ...freeSpot(state, 1), hidden: false }); changed = true; });
  }
  return changed;
}
