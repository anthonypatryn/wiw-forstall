...
import { KINDS, SLOTS, fields, inRange, bestSweep, lossText, edisonConflicts, hexDist as fsDist } from './forstall.js';
...
export function hexDist(a, b) { const A = cube(a.col, a.row), B = cube(b.col, b.row); return Math.max(Math.abs(A.q - B.q), Math.abs(A.r - B.r), Math.abs(A.s - B.s)); }
...
export function autoSync(state, combat) {
  ...
  if (combat.combat?.active) {
    ...
    (combat.posse || []).filter((p) => !p.dead && (!party || party.includes(p.id)) && !have.has(p.id)).forEach((p) => { state.tokens.push({ id: crypto.randomUUID().slice(0, 8), kind: 'pc', ref: p.id, name: p.name, ...freeSpot(state, rows - 2), hidden: false }); changed = true; });
    (combat.enemies || []).filter((e) => !e.defeated && !e.out && !have.has(e.id)).forEach((e) => { state.tokens.push({ id: crypto.randomUUID().slice(0, 8), kind: 'enemy', ref: e.id, name: e.name, ...freeSpot(state, 1), hidden: false }); changed = true; });
  }
  return changed;
}
...
export function battleView(state, { warden, combat }) {
  ...
  return {
    ...
    forstalls: fl.filter((f) => f.pos && (warden || !f.hidden)).map(({ known, ...f }) => ({
      ...f, known: [...known],
      ...
      pulse: f.heartbeat && f.sweep ? (() => {
        const d = (combat?.enemies || []).filter((e) => !e.defeated).map((e) => state.tokens.find((t) => t.kind === 'enemy' && t.ref === e.id)).filter(Boolean)
          .map((t) => fsDist(f.pos, t)).filter((x) => f.rangeIn >= 999 || x <= f.rangeIn + 6);
