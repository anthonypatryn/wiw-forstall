...
const say = (L, text) => { L.log.push(text); L.log = L.log.slice(-40); };
...
// Intuition vs the NPC's Finesse: see one of their dice
export function liarsPeek(t, seat, a, ctx, challenge) {
  ...
  if (!target || target.kind !== 'npc' || !(L.counts[target.key] > 0)) throw new Error('Pick an NPC still holding dice.');
...
// Charm vs the next NPC's Intuition, right after your bid: they have to raise, not call
export function liarsStare(t, seat, ctx, challenge) {
  ...
  if (!next || next.kind !== 'npc') throw new Error('The next player isn’t an NPC.');
...
export function liarsView(t, { warden, mine }) {
  ...
  return {
    ...
    all: warden ? L.dice : undefined, stared: warden ? L.stared : undefined,
  };
}
