...
const say = (D, text) => { D.log.push(text); D.log = D.log.slice(-40); };
const seatOf = (t, k) => t.seats.find((s) => s.key === k);
export const SHOT = 0.1; // the catalog's whiskey, per pour
export const PASS_OUT = 3;
const NPC_GRIT = { Weak: 2, Moderate: 4, Strong: 6 };

const pcOf = (seat, ctx) => ctx.posse.find((p) => p.id === seat.pc);
function walletOf(seat, ctx) { return seat.kind === 'npc' ? seat.bank : money(pcOf(seat, ctx)?.wallet); }
