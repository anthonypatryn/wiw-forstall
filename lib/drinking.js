...
import { say, seatOf, pcOf, walletOf, add, sharpest } from './saloon-common.js';
...
// average Hits per die (B: blank, blank, spur, hit, hit, ace=2 → 4/6; G: blank, spur, hit, hit, hit, ace=2 → 5/6) — how NPCs judge a shot
const HITS_PER_BLACK = 4 / 6, HITS_PER_GOLD = 5 / 6;
...
function drinkShot(t, seat, grit, ctx) {
  ...
  if (seat.kind === 'npc') { // NPCs spend Grit when their dice look short of what they need
    ...
    const expect = p.black * HITS_PER_BLACK + p.gold * HITS_PER_GOLD;
    grit = Math.min(3, D.grit[k] || 0, Math.max(0, Math.ceil((need - expect) / HITS_PER_BLACK)));
...
// Finesse vs the sharpest eye still upright (posse or NPC): dump your shot this round. Caught = you drink a double
export function spittoon(t, seat, ctx, challenge) {
  ...
  const eye = sharpest(upright(D).map((x) => seatOf(t, x)).filter((s) => s && s.key !== k), ctx); // anyone still upright, posse or NPC
...
// Charm vs anyone's Nerve: they roll 2 fewer dice on their next shot
export function needle(t, seat, a, ctx, challenge) {
  ...
  if (!target || target.key === k || !D.order.includes(target.key) || D.out[target.key]) throw new Error('Pick someone still upright.');
