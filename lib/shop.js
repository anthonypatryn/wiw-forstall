...
import { equipItem, isPlaced, placedIn } from './sheets.js';
...
export function shopView(shop, { warden, combat }) {
  return {
    ...
    posse: (combat?.posse || []).map((p) => ({ id: p.id, name: p.name, trade: p.trade, wallet: walletOf(p), items: (p.items || []).map((it) => ({ ...it, placed: placedIn(p, it) })) })),
  };
}
