...
export function freshState() {
  return { v: 0, active: null, settings: { easyMode: false }, custom: [], notebook: {} };
}
...
export function doRoll(state, { black, gold, spurTalent, operator }) {
  ...
  if (m.scanHalf) pool = halvePool(pool.black, pool.gold);
  ...
  const roll = {
    at: now(), operator: cleanName(operator), requested, pool, halved: !!m.scanHalf,
...
export function doGuess(state, { digits, operator }) {
  const m = state.active && findMonster(state, state.active);
  ...
  entry.guesses.push({ at: now(), operator: cleanName(operator), digits, result });
...
export function wardenAction(state, a) {
  switch (a.action) {
    ...
    case 'addCustom': {
      ...
      state.custom.push({ name, kz: formatKz(d), size: a.size || 'Medium', page: null });
      return;
    }
    case 'removeCustom':
