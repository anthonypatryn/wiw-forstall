...
import { rollPool, clampDice } from './dice.js';
...
const formatKz = (d) => `${d[0]}-${d[1]}-${d.slice(2).join('')}`;
...
function findMonster(state, name) {
...
export function doRoll(state, { black, gold, spurTalent }) {
  ...
  const { dice, hits } = rollPool(pool.black, pool.gold, !!spurTalent);
...
export function playerView(state) {
  ...
  return {
    ...
    pending: state.pending ? { pc: state.pending.pc, name: state.pending.name } : null, // a fight Scan waiting on its one guess
    notebook,
  };
}
