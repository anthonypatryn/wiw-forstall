...
export const BOOK_TOWNS = PLACES.filter((p) => ['town', 'settlement', 'rail'].includes(p.kind)).map((p) => ({ id: p.id, name: p.name, x: p.x, y: p.y }));
...
export function wantedAction(state, a, { warden, towns = [], posse = [], log = () => {} }) {
  ...
  switch (a.action) {
    ...
    case 'remove': find(); state.posters = state.posters.filter((p) => p.id !== a.id); return;
