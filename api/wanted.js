...
import { freshWanted, wantedAction, wantedView, allTowns, TERMS } from '../lib/wanted.js';
import { freshCombat, addLog } from '../lib/combat.js';
import { freshMap } from '../lib/map.js';
...
export default async function handler(req, res) {
  try {
    ...
    const view = (asWarden) => {
      ...
      return {
        ...v, v: `${state.v}.${map.v || 0}`, here: hereTown(towns, map, combat.posse), terms: TERMS,
    ...
    const result = wantedAction(state, body, { warden, towns: allTowns(state, map.pins || []), posse: combat.posse, log }) ?? null;
