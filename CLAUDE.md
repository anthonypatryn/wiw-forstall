...
- **Lockpicks:** a house item in the catalog (`general-goods-tools-lockpick`, $0.50, `house: true`, shown as "House item" in the Store). A retry whose cost mentions a lockpick takes one out of the picker's inventory (`api/lockpick.js`), or refuses if they have none. **Locksmith** (Trapper, 2/day) adds +2B to the Finesse peek roll and uses one of its daily uses.
...
- **Loot & traps:** `start` also takes `loot {kind: money|scrap|item|custom, amount | itemId | name+desc}` and `trap {damage, status, sev}` (hurts whoever opens it). Both stay hidden from players (`lockView`) until `picked`; then `lockAction` calls `onOpen(a, 'trap'|'loot')` once each, and `api/lockpick.js` applies them to the picker automatically — trap via `publicAction` health/status, Money → `wallet`, Scrap → `scrap`, store item → inventory + `equipItem`, new item → inventory (`sub: 'Found'`, `note` = description). `a.sprung` / `a.found` carry the result text for the scene and Table Log.
